...
const typeDefs = await readFile(
  path.resolve("./src/schema.graphql"), // point to the source file
  "utf8"
);
/**
 * Apollo context provider
 */
async function getContext({ req }: { req: Request }): Promise<GraphQLContext> {
  const companyLoader = createCompanyLoader();
  const context: GraphQLContext = { companyLoader };
